...
export default {
  ...
  computed: {
    ...
    // TODO add in the below active kev
    activeFilters () {
  ...
  async asyncData ({ store, route, context }) { // this is for SSR purposes to prefetch data
    Logger.info('Entering asyncData in Seller Page (core)')()
    try {
      ...
      if (!query.searchProductQuery) {
        store.dispatch('category/mergeSearchOptions', {
          // searchProductQuery: baseFilterProductsQuery(parentCategory, defaultFilters, typeof route.query.term !== 'undefined' ? route.query.term : null)
          // TODO upgrade system with this changes kev
          searchProductQuery: baseFilterProductsQuery(parentCategory, defaultFilters, typeof route.query.term !== 'undefined' ? route.query.term : 'iphone')
        })
      }
      // Remove Category & Add Seller Search
      remove(query.searchProductQuery._appliedFilters, {
        attribute: 'category_ids'
      })
      const sellerSearch = {'attribute': 'seller_id', 'value': {'in': route.params.slug}, 'scope': 'catalog'}
      query.searchProductQuery._appliedFilters.push(sellerSearch)
  ...
  async beforeRouteEnter (to, from, next) {
    if (!isServer && !from.name) { // Loading category products to cache on SSR render
      next(vm => {
        ...
        const sellerSearch = {'attribute': 'seller_id', 'value': {'in': to.params.slug}, 'scope': 'catalog'}
        query.searchProductQuery._appliedFilters.push(sellerSearch)
  ...
  methods: {
    ...
    pullMoreProducts () {
      ...
      let query = this.getCurrentCategoryProductQuery
      remove(query.searchProductQuery._appliedFilters, {
        attribute: 'category_ids'
      })
      const sellerSearch = {'attribute': 'seller_id', 'value': {'in': this.$route.params.slug}, 'scope': 'catalog'}
      query.searchProductQuery._appliedFilters.push(sellerSearch)
      if (query.current <= this.productsTotal) {
        this.mergeSearchOptions({
          searchProductQuery: buildFilterProductsQuery(this.category, this.filters.chosen)
        })
        return this.$store.dispatch('category/products', query)
      }
    },
    onFilterChanged (filterOption) {
      ...
      let filterQr = buildFilterProductsQuery([2], this.filters.chosen)
      ...
      const sellerSearch = {'attribute': 'seller_id', 'value': {'in': this.$route.params.slug}, 'scope': 'catalog'}
      query.searchProductQuery._appliedFilters.push(sellerSearch)
      this.$store.dispatch('category/products', query).then((res) => {
