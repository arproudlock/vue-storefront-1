...
export default {
  ...
  async asyncData ({ store, route, context }) { // this is for SSR purposes to prefetch data
    Logger.info('Entering asyncData in Brand Page (core)')()
    try {
      ...
      // Remove Category & Add Brand Search
      remove(query.searchProductQuery._appliedFilters, {
        attribute: 'category_ids'
      })
      const brandSearch = {'attribute': 'brand', 'value': {'in': route.params.slug}, 'scope': 'catalog'}
      query.searchProductQuery._appliedFilters.push(brandSearch)
  ...
  async beforeRouteEnter (to, from, next) {
    if (!isServer && !from.name) { // Loading category products to cache on SSR render
      next(vm => {
        ...
        const brandSearch = {'attribute': 'brand', 'value': {'in': to.params.slug}, 'scope': 'catalog'}
        query.searchProductQuery._appliedFilters.push(brandSearch)
  ...
  methods: {
    ...
    pullMoreProducts () {
      ...
      const brandSearch = {'attribute': 'brand', 'value': {'in': this.$route.params.slug}, 'scope': 'catalog'}
      query.searchProductQuery._appliedFilters.push(brandSearch)
