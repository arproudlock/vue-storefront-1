...
export default {
  ...
  methods: {
    ...
    selectVariant () {
      console.log('select variant triggered');
      if (config.products.gallery.mergeConfigurableChildren) {
        ...
        if (option) {
          ...
          let index = this.gallery.findIndex(
            ...
            console.log(index);
          if (index < 0) index = this.gallery.findIndex(obj => obj.id && obj.id.colour === option.colour)
          this.navigate(index)
          console.log('navigate triggered, index: '+index);
        }
      }
    },
    openOverlay () {
    ...
    pageChange (index) {
      ...
      this.$bus.$emit('carousel-page-change', index)
